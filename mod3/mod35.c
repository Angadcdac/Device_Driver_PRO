...
char *kbuf;
char *mymessage = "This from  the  Kernel";
// function prototypes
...
//	if ( copy_to_user(buf, mymessage + *off, len) )//send kernel message to user
	if ( copy_to_user(buf, kbuf + *off, len) ) //send the recived message to user
	{
		pr_err("Error in data write to userspace!\n");
	}
	pr_info("The send data to user space is ==%s",kbuf);
...
static ssize_t my_write(struct file *file, const char __user *buf, size_t len, loff_t *off)
{
	...
	pr_info("Received data from the user space is ==%s",kbuf);
...
/*
 * my_read() - Driver read function
 *
 * This function is called when a user application invokes read()
 * on the device file (/dev/cdac_dev).
 *
 * It transfers data from kernel space (kbuf) to user space (buf).
 *
 * copy_to_user() - Copy data from kernel to user space
 *
 * buf         : Destination (user-space buffer)
 * kbuf + *off : Source (kernel buffer + current offset)
 * len         : Number of bytes to copy
 *
 * Returns:
 *  0 → success
 *  non-zero → number of bytes NOT copied (error)
 */
/*****************************************************************************************************/	
/*  static ssize_t my_write(struct file *file, const char __user *buf, size_t len, loff_t *off)
    if ( copy_from_user(kbuf, buf, len) )

 * my_write() - Driver write handler
 *
 * This function is invoked when a user application calls write()
 * on the device file (/dev/cdac_dev).
 *
 * It copies data from user space (buf) to kernel space (kbuf).
 *
 * copy_from_user() - safely copy data from user to kernel
 *
 * kbuf : destination (kernel buffer)
 * buf  : source (user-space buffer)
 * len  : number of bytes to copy
 *
 * Return:
 *  0      → success
