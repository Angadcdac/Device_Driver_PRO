...
#define SIZE	(5)
static long myarr[SIZE] = {1,2,3,4,5};
static int count = 5; // will contain how many array entries were received

// register them with the kernel
// perms: all can read, only USR (root) can write/edit
module_param(myint, int, S_IRUGO | S_IWUSR);      //register int  with the kernel 
module_param(mycharp, charp, S_IRUGO | S_IWUSR);  //resister string with the kernel 
module_param_array(myarr, long, &count, S_IRUGO | S_IWUSR); //register array with the kernel 
