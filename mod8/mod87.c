...
int wthread_func(void *pv)
{
        ...
        {
                write_seqlock(&my_seqlock);
                // Acquire seqlock for writing (exclusive access).
                // Internally increments sequence number to an ODD value → indicates write in progress.

                sprintf(buffer, "%s:%d->%d", wthread_name, count++, count);
                // Update shared data (count).
                // Only writer modifies data → safe inside seqlock.

                pr_info("In thread %s, got seqlock, buffer=%s\n", __func__, buffer);

                write_sequnlock(&my_seqlock);
                // Release seqlock.
                // Sequence number becomes EVEN → indicates write completed.

                msleep(WTIME_INTVL);
                // Sleep outside critical section (correct usage).
        }
        return 0;
}
*/
...
int rthread_func(void *pv)
{
        ...
        while(!kthread_should_stop())
        {
                do
                {
                        ...
                        // If ODD → writer in progress.

                        pr_info("Read try - seq_no=%d", seq_no);

                        read_value = count;
                        // Read shared data WITHOUT locking.
                        // Fast, no blocking.
                }
                while (read_seqretry(&my_seqlock, seq_no));
                // Retry if:
                // 1. Sequence number changed during read
                // 2. Sequence number was ODD (writer active)

                pr_info("Read done - seq_no=%d", seq_no);

                sprintf(buffer, "%s:%d", thread_name, count);
                // NOTE: Ideally should use read_value instead of count
