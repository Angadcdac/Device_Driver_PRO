/*
2. Write a user space application that performs open, write, read and close operations on the character device so created above. The user space data should be written and read from an appropriate variable in kernel memory.
...
#define MY_MAJOR_NUM	(202)//(250)//NOt allowing
#define MY_CLASS_NAME	"my_cls"
#define MY_DEV_NAME	"my_char_dev"
#define MY_KBUF_SZ	(10)
...
static int __init mod_init(void)
{
	...
	kbuf = (char *)kmalloc(MY_KBUF_SZ, GFP_KERNEL);//GFP_KERNAL....not an emergency you an sleep
	if (!kbuf)
	{
	...
	if (IS_ERR(dev_class))
	{
		...
		goto r_class;			//use goto to remove code complexity
	}
...
r_device:			//unrolling........error ladder....destory all things in the revsrese order
...
r_kbuf:
	kfree(kbuf);	//free the memory allocated by kmalloc to kbuf by kernal
	
	return -1;		//signal to the kernal that error in loading module....means insmod is failed
}

static void __exit mod_exit(void)
{
	pr_info("Goodbye world!!\n");
	device_destroy(dev_class, dev);	//destroying in reverse manner....rmmod
	class_destroy(dev_class);
	cdev_del(&my_cdev);
	unregister_chrdev_region(dev, 1);
	kfree(kbuf);
